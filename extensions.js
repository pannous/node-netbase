...
ƒ=false
ø=[]
ℕ=function*(){i=1;while(ı){yield i;i+=1}}
// ∞ error
...
hex=x=>x.toString(16)
print=p=function(x){console.log(x);return x};
dir=function(x){print(typeof x); return Object.getOwnPropertyNames(x)}
// dir=xs=>{for(key in xs){if (!xs.hasOwnProperty(key))continue;console.log(key+":"+xs[key])}}
all=(xs)=>{for(key in xs)console.log(key+":"+xs[key])}
// map=(f,xs)=>xs.map(f)  
map=(f,xs)=>{for(key in xs){if(xs.hasOwnProperty(key))xs[key]=f(xs[key])}}
do_map=(f,xs)=>{for(key in xs){if(xs.hasOwnProperty(key))xs[key]=f(xs[key])}}
is_file =exists=(path)=>{try{return fs.existsSync(path)}catch(x){return 0}}


keys=function(x){return Object.keys(x)}
len=function(x){console.log("use .length");return x.length}
rand=random=function(x){return x?int(Math.random()*x):Math.random()}
randi=randint=function(x){return x?int(random(x)):int(random(Number.MAX_SAFE_INTEGER))}
...
Object_Extensions={// FUCKS UP SEQUALIZE!! and probably others
...
Array_Extensions={
  ...
  append(x) { this[this.len] = (x); return this},// vs concat: not selfmodifying!
  insert(x) { return this[this.len] = (x); return this},
  add(x) { return this[this.len] = (x); return this},
  strip(){return this.filter(x=>x.length>0)},
  sub(x, y) { echo('use slice'); return this.slice(x, y) }, // substring  cannot handle negative values!!
  merge(x){return this.concat(x)},// NOT selfmodifying
  values(x){return this}, // WHY??? ~ clone?  <> Object.values(x)
  clone(){return this.slice()},
  each(x){return this.forEach(x)},
  deduplicate() { return this.filter((item, pos) => this.indexOf(item) == pos) },
  dedupe(){return this.filter((item, pos)=>this.indexOf(item) == pos)},
  unique(){return this.filter((item, pos)=>this.indexOf(item) == pos)},
  ...
  contains(x){return this.includes(x)},// ES7 OK!!
...
String_Extensions={// StringExtensions
  capitalizeFirstLetter() {return this.charAt(0).toUpperCase() + this.slice(1);},
  capitalize(){return this[0].toUpperCase()+this.slice(1)},
  in(xs){return xs.indexOf(this)>=0},
  has(x){return this.match(x)},
  unicode_length(){return Array.from(this).length},
  chars(){return Array.from(this)},// unicode OK
  charsASCII(){return this.split('')},// ascii
  at(i){return unicode(this.codePointAt(i))},
  grep(x){return this.split("\n").grep(x)},
  g(x){return this.split("\n").grep(x)},
  sub(x,y){return this.slice(x,y)}, // substring  cannot handle negative values!!
  ...
  uppercase(){echo("toUpperCase!");return this.toUpperCase()},
  lowercase(){echo("toLowerCase!");return this.toLowerCase()},
  replaceAll(a,b){return this.replace(new RegExp(a,"gi"),b)},
  hasChars(){return len(this.replace(/[^a-zA-Z]/g,""))>0},
  reverse(){return Array.from(this.normalize()).reverse().join("")}
}
...
Reflect_Extensions={ //_Extensions
  toSource(){return Reflect.stringify(this, "  ");}, // AST to source !!
  eval(){return eval(Reflect.stringify(this, "  "))},
  run(){return eval(Reflect.stringify(this, "  "))},
}
...
// Object.assign(Object.prototype,MyObject) // merge into!
    ...
    try{ Object.assign(String.prototype,String_Extensions)}catch(exc){}//error(exc)}}
    try{ Object.assign(HTMLCollection.prototype,HTMLCollection_Extensions)    }catch(exc){}//error(exc)}
    ...
    Object.defineProperty(Array.prototype,'count', { get(){return this.length},configurable:true /* READ-ONLY*/ });
    Object.defineProperty(String.prototype,'len', { get(){return  Array.from(this).length},configurable:true /* READ-ONLY*/ });// UNICODE CONFORM!
    Object.defineProperty(String.prototype,'length_unicode', { get(){return  Array.from(this).length},configurable:true /* READ-ONLY*/ });// UNICODE CONFORM!
    ...
    Object.defineProperty(String.prototype, 'upper', { get() { return this.toUpperCase() }, configurable: true });
    Object.defineProperty(String.prototype, 'lower', { get() { return this.toLowerCase() }, configurable: true });
...
run=exec=execSync=sys=system=x=>child_process.execSync(x).toString().split('\n')
...
function yield(gen){return gen.next().value}
...
wget=require('deasync')(fetch)
// console.log(wget("http://mimi.com"));
// await fetch("http://mimi.com");

downloadSync=wget=(url)=>{
  ret = undefined
  fetch(url).then(response=>response.text().then(data=>ret=data))
